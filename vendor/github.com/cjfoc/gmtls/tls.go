...
var (
	default_uid = []byte{0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38} // 加密机固定uid值
)
...
func parsePrivateKey(der []byte) (crypto.PrivateKey, error) {
	if key, err := crx509.ParsePKCS1PrivateKey(der); err == nil {
		return key, nil
	}
	if key, err := crx509.ParsePKCS8PrivateKey(der); err == nil {
		switch key := key.(type) {
