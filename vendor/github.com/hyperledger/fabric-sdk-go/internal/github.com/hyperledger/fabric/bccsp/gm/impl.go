...
func (csp *impl) KeyGen(opts bccsp.KeyGenOpts) (k bccsp.Key, err error) {
	// Validate arguments
	if opts == nil {
		return nil, errors.WithMessage(err, "Invalid Opts parameter. It must not be nil.")
	}

	keyGenerator, found := csp.keyGenerators[reflect.TypeOf(opts)]
	if !found {
		return nil, errors.Wrapf(err, "Unsupported 'KeyGenOpts' provided [%v]", opts)
	}
	...
	if !opts.Ephemeral() {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "Failed storing key [%s]. [%s]", opts.Algorithm())
		}
	}
...
func (csp *impl) KeyDeriv(k bccsp.Key, opts bccsp.KeyDerivOpts) (dk bccsp.Key, err error) {
	// Validate arguments
	if k == nil {
		return nil, errors.Wrapf(err, "Invalid Key. It must not be nil.")
	}
	if opts == nil {
		return nil, errors.Wrapf(err, "Invalid opts. It must not be nil.")
	}

	keyDeriver, found := csp.keyDerivers[reflect.TypeOf(k)]
	if !found {
		return nil, errors.Wrapf(err, "Unsupported 'Key' provided [%v]", k)
	}
	...
	if !opts.Ephemeral() {
		...
		err = csp.ks.StoreKey(k)
		if err != nil {
			return nil, errors.Wrapf(err, "Failed storing key [%s]. [%s]", opts.Algorithm(), err)
		}
	}
...
func (csp *impl) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (k bccsp.Key, err error) {
	// Validate arguments
	if raw == nil {
		return nil, errors.Wrapf(err, "Invalid raw. It must not be nil.")
	}
	if opts == nil {
		return nil, errors.Wrapf(err, "Invalid opts. It must not be nil.")
	}
	...
	if !found {
		return nil, errors.Wrapf(err, "Unsupported 'KeyImportOpts' provided [%v]", opts)
	}
...
func (csp *impl) Hash(msg []byte, opts bccsp.HashOpts) (digest []byte, err error) {
	...
	if !found {
		return nil, errors.Wrapf(err, "Unsupported 'HashOpt' provided [%v]", opts)
	}
...
func (csp *impl) Sign(k bccsp.Key, digest []byte, opts bccsp.SignerOpts) (signature []byte, err error) {
	...
	if len(digest) == 0 {
		return nil, errors.Wrapf(err, "Invalid digest. Cannot be empty.")
	}

	signer, found := csp.signers[reflect.TypeOf(k)]
	if !found {
		return nil, errors.Wrapf(err, "Unsupported 'SignKey' provided [%v]", k)
	}
...
func (csp *impl) Verify(k bccsp.Key, signature, digest []byte, opts bccsp.SignerOpts) (valid bool, err error) {
	// Validate arguments
	if k == nil {
		return false, errors.Wrapf(err, "Invalid Key. It must not be nil.")
	}
	if len(signature) == 0 {
		return false, errors.Wrapf(err, "Invalid signature. Cannot be empty.")
	}
	if len(digest) == 0 {
		return false, errors.Wrapf(err, "Invalid digest. Cannot be empty.")
	}

	verifier, found := csp.verifiers[reflect.TypeOf(k)]
	if !found {
		return false, errors.Wrapf(err, "Unsupported 'VerifyKey' provided [%T]", k)
	}
...
func (csp *impl) Encrypt(k bccsp.Key, plaintext []byte, opts bccsp.EncrypterOpts) (ciphertext []byte, err error) {
	...
	if !found {
		return nil, errors.Wrapf(err, "Unsupported 'EncryptKey' provided [%v]", k)
	}
...
func (csp *impl) Decrypt(k bccsp.Key, ciphertext []byte, opts bccsp.DecrypterOpts) (plaintext []byte, err error) {
	...
	if !found {
		return nil, errors.Wrapf(err, "Unsupported 'DecryptKey' provided [%v]", k)
	}
